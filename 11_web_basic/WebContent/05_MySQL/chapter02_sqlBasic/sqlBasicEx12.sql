/*
         ...
         #  GROUP BY
			...
			- 출력된 레코드를 그룹으로 묶고 각 그룹에대한 요약 값을 계산한다. ( 집계함수와 같이 사용 한다. )
...
# 'user1' , 'user2' , 'user3' 의 주문건수 조회하기 (3유저의 주문건수(O), 3유저별 주문건수(X))
SELECT
		COUNT(*)
FROM
		ORDER_TB
...
# 'product1' , 'product3' , 'product5' 의 주문건수 조회하기  (3유저의 주문개수(O), 3유저별 주문개수(X))
SELECT
		*
FROM
		ORDER_TB
